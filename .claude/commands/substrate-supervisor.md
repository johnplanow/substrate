...
```bash
npx substrate supervisor --experiment --output-format json --poll-interval 30 &
SUPERVISOR_PID=$!
```
...
```bash
npx substrate supervisor --output-format json --poll-interval 30 | while IFS= read -r line; do
...
1. Run `substrate status --output-format json` to see which stories are stuck
