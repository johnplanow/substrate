 ...
 *
 * Implements 9 rules across three categories:
 ...
 *   - Efficiency rules: cache_efficiency, per_model_comparison, cache_delta_regression
 *
 * Architecture constraints:
 *   - Pure analysis service — no DB access; caller assembles RecommenderContext
 *   - Constructor injection: accepts ILogger (defaults to console)
  ...
  private _runRepeatedToolCalls(ctx: RecommenderContext): Recommendation[] {
    ...
    for (const turn of turns) {
      for (const child of turn.childSpans) {
        const entry: { toolName?: string; name: string; actionTarget?: string } = {
          name: child.name,
        }
        if (child.toolName !== undefined) entry.toolName = child.toolName
        allChildSpans.push(entry)
      }
    }
    ...
    if (allChildSpans.length === 0 && allSpans.length > 0) {
      for (const span of allSpans) {
        const spanToolName = span.attributes?.['tool.name'] as string | undefined
        const spanFilePath = span.attributes?.['file.path'] as string | undefined
        const entry: { toolName?: string; name: string; actionTarget?: string } = {
          name: span.name,
        }
        if (spanToolName !== undefined) entry.toolName = spanToolName
        if (spanFilePath !== undefined) entry.actionTarget = spanFilePath
        allChildSpans.push(entry)
      }
    }
