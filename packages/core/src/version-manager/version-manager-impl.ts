...
export class VersionManagerImpl implements VersionManager {
  ...
  isConfigCompatible(configVersion: string): boolean {
    return (SUPPORTED_CONFIG_FORMAT_VERSIONS as readonly string[]).includes(configVersion)
  }
  ...
  isTaskGraphCompatible(graphVersion: string): boolean {
    return (SUPPORTED_TASK_GRAPH_VERSIONS as readonly string[]).includes(graphVersion)
  }
}
