 ...
 */
export interface GitWorktreeManager extends IBaseService {
  /**
   ...
   *
   * @param taskId       - The task identifier
   * @param targetBranch - The branch to merge into (defaults to 'main')
   * @returns            - ConflictReport with hasConflicts and conflictingFiles
   * @throws             - Error if the worktree for this task does not exist
   */
  detectConflicts(taskId: string, targetBranch?: string): Promise<ConflictReport>

  /**
   * Merge a task's worktree branch into a target branch.
   *
   * @param taskId       - The task identifier
   * @param targetBranch - The branch to merge into (defaults to 'main')
   * @returns            - MergeResult with success status and merged files
   * @throws             - Error if merge command fails unexpectedly
   */
   ...
   *
   * @returns - Array of WorktreeInfo for all tracked worktrees
   */
