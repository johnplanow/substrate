...
describe('Supersession Detection → Delta Document Integration (Stories 12-9 + 12-10)', () => {
  ...
  it('decisions superseded by runPostPhaseSupersessionDetection appear in the delta document supersededDecisions', async () => {
    ...
    const parentD1Row = adapter.querySync<{ superseded_by: string | null }>(
  ...
  it('multiple supersessions across phases all appear in delta document', async () => {
    ...
    insertDecision(adapter, pD3, parentRunId, {
      phase: 'solutioning',
      category: 'api',
      key: 'protocol',
      value: 'REST',
    ...
    insertDecision(adapter, aD2, amendmentRunId, {
      phase: 'planning',
      category: 'scope',
      key: 'deadline',
      value: 'Q2',
    })
    insertDecision(adapter, aD3, amendmentRunId, {
      phase: 'solutioning',
      category: 'api',
      key: 'protocol',
      value: 'GraphQL',
    ...
    const mockDispatch = async (_prompt: string): Promise<string> => {
      return JSON.stringify([
        ...
        },
        {
          confidence: 'MEDIUM',
          area: 'API Surface',
          description: 'May affect clients',
          relatedDecisionIds: [aD1],
        },
      ])
    }
...
describe('runPostPhaseSupersessionDetection: real DB + handler state integration', () => {
  ...
  it('correctly identifies matching decisions by phase + category + key tuple', async () => {
    ...
    insertRun(adapter, parentRunId, 'completed')
    insertDecision(adapter, pMatchD, parentRunId, {
      phase: 'analysis',
      category: 'arch',
      key: 'db',
      value: 'MySQL',
    })
    insertDecision(adapter, pNoMatchD, parentRunId, {
      phase: 'analysis',
      category: 'arch',
      key: 'cache',
      value: 'Redis',
    ...
    insertDecision(adapter, aMatchD, amendmentRunId, {
      phase: 'analysis',
      category: 'arch',
      key: 'db',
      value: 'PostgreSQL',
    ...
    const pNoMatchRow = adapter.querySync<{ superseded_by: string | null }>(
      'SELECT superseded_by FROM decisions WHERE id = ?',
      [pNoMatchD]
    )[0]
  ...
  it('only processes decisions from the specified phase — cross-phase decisions are not superseded', async () => {
    ...
    insertRun(adapter, parentRunId, 'completed')
    insertDecision(adapter, pAnalysisD, parentRunId, {
      phase: 'analysis',
      category: 'arch',
      key: 'db',
      value: 'MySQL',
    })
    insertDecision(adapter, pPlanningD, parentRunId, {
      phase: 'planning',
      category: 'arch',
      key: 'db',
      value: 'MySQL',
    ...
    const aD = randomUUID()
    insertDecision(adapter, aD, amendmentRunId, {
      phase: 'analysis',
      category: 'arch',
      key: 'db',
      value: 'PostgreSQL',
    ...
    const pPlanningRow = adapter.querySync<{ superseded_by: string | null }>(
      'SELECT superseded_by FROM decisions WHERE id = ?',
      [pPlanningD]
    )[0]
  ...
  it('errors in individual supersedeDecision() calls do not halt the detection loop', async () => {
    ...
    const pD2Row = adapter.querySync<{ superseded_by: string | null }>(
      'SELECT superseded_by FROM decisions WHERE id = ?',
      [pD2]
    )[0]
  ...
  it('returns void and handler state updates correctly across multiple phase calls', async () => {
    ...
    insertRun(adapter, parentRunId, 'completed')
    insertDecision(adapter, pA, parentRunId, {
      phase: 'analysis',
      category: 'arch',
      key: 'db',
      value: 'MySQL',
    })
    insertDecision(adapter, pB, parentRunId, {
      phase: 'planning',
      category: 'scope',
      key: 'deadline',
      value: 'Q1',
    ...
    const aA = randomUUID()
    const aB = randomUUID()
    insertDecision(adapter, aA, amendmentRunId, {
      phase: 'analysis',
      category: 'arch',
      key: 'db',
      value: 'PostgreSQL',
    })
    insertDecision(adapter, aB, amendmentRunId, {
      phase: 'planning',
      category: 'scope',
      key: 'deadline',
      value: 'Q2',
    ...
    const result1 = await runPostPhaseSupersessionDetection(
      adapter,
      amendmentRunId,
      'analysis',
      handler
    ...
    const result2 = await runPostPhaseSupersessionDetection(
      adapter,
      amendmentRunId,
      'planning',
      handler
