...
export class AdapterTelemetryPersistence implements ITelemetryPersistence {
  ...
  // packages/core/src/persistence/schema.ts. Ship 4 (v0.20.95) removed the
  // duplicate DDL that used to live here — the same 5 telemetry tables
  // (turn_analysis, efficiency_scores, recommendations, category_stats,
  // consumer_stats) + their indexes + ALTER migrations are now defined
  // exclusively in initSchema. Callers MUST call the main initSchema(adapter)
  // before constructing AdapterTelemetryPersistence.
