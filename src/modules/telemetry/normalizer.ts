...
export class TelemetryNormalizer {
  ...
  private _normalizeOneSpan(
    ...
    // Token extraction
    const fromAttrs = extractTokensFromAttributes(span.attributes as Parameters<typeof extractTokensFromAttributes>[0])
    const bodyStr = getAttrString(span.attributes, 'llm.response.body') ??
    ...
    // Story key
    const storyKey = getAttrString(span.attributes, 'substrate.story_key') ??
  ...
  private _normalizeOneLog(
    ...
    const fromAttrs = extractTokensFromAttributes(record.attributes as Parameters<typeof extractTokensFromAttributes>[0])
    const fromBody = extractTokensFromBody(bodyStr)
    const tokens = mergeTokenCounts(fromAttrs, fromBody)
