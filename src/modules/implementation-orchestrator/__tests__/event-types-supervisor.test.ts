...
describe('AC4: generateCommandReferenceSection — metrics --analysis flag', () => {
  it('documents --analysis <run-id> flag for substrate auto metrics', () => {
    const output = generateCommandReferenceSection()
    expect(output).toContain('--analysis')
    expect(output).toContain('substrate auto metrics')
  })

  it('documents --experiment flag for substrate auto supervisor', () => {
    const output = generateCommandReferenceSection()
    expect(output).toContain('--experiment')
    expect(output).toContain('substrate auto supervisor')
  })

  it('documents --max-experiments flag for substrate auto supervisor', () => {
    const output = generateCommandReferenceSection()
