...
// (Ship 9, v0.20.100) Branch-lifecycle tests removed.
// `branchForStory`/`mergeStory`/`rollbackStory` were decommissioned because
// the orchestrator wired them via fire-and-forget calls on the StateStore,
// but the actual implementation lived only in FileStateStore (no-op stubs)
// and DoltStateStore (unreachable — the orchestrator uses FileStateStore).
// The Dolt branch-per-story scheme (Epic 26) was superseded by the
// substrate-worktrees + git-branch dispatch path (v0.20.79+ Epic 75).
