...
describe('TOKEN_CEILING_DEFAULTS', () => {
  it('exports the expected hardcoded default values', () => {
    expect(TOKEN_CEILING_DEFAULTS['create-story']).toBe(3000)
    expect(TOKEN_CEILING_DEFAULTS['dev-story']).toBe(24_000)
    expect(TOKEN_CEILING_DEFAULTS['code-review']).toBe(100_000)
    expect(TOKEN_CEILING_DEFAULTS['test-plan']).toBe(8_000)
    expect(TOKEN_CEILING_DEFAULTS['test-expansion']).toBe(20_000)
  })
})
