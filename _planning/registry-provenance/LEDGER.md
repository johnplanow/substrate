...
Ship 3 (v0.21.15) landed — RP1 COMPLETE (derive → ratify → diff, all live-verified). Ship 3 verified ⇒ [dogfood-eligible] unlocked (single-story only). Start at RP2.1 (Ship 4): staleness — `validate` + orchestrator acceptance preflight re-hash `provenance.derived_from` → `registry-stale` advisory (both hashes + re-derive hint); `registry-source-missing` distinct advisory; path containment (reject traversal); matrix cell `registry-stale`.
