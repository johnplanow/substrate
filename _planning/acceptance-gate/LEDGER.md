...
Ships 1–3 SHIPPED (v0.21.2–v0.21.4) — Epic A0 complete + A1.1/A1.2 done. Next: A1.3 (spec-tamper + fixture-mutation tripwires) — Ship 4. Seams: TestMutationCheck (H1.7 pattern), ground-truth changedFiles plumbing (H1.5), `loadJourneyRegistryFromTrustedTree` vs worktree-copy compare. Carry-over from A0.3: deferrals trusted-read uses HEAD at run end (story-merged edits could tamper) — A1.3 must cover deferrals.yaml alongside journeys.yaml and the profile acceptance block.
